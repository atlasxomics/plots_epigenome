...
gvol_cluster = w_select(
    ...
    appearance={
        "help_text": "Subselect data to a specific cluster."
    }
)
...
if len(gvol_df) == 0:
    w_text_output(
       content=f"There is no volcano plot for cluster {gvol_cluster.value} \
            because it contains more than 90% of one of the conditions. \
            Please check Proportion plot.",
       appearance={"message_box": "warning"}
    )
    exit(0)
