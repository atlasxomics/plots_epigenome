...
if construct_button.value:
    ...
    if score_cols:
        ...
        if not skip_unassigned_checkbox.value:
            low_conf_mask = adata_subset.obs["pred_cell_type_conf"] < 0.1
    ...
    # Store results in chosen metadata column
    if result_label_select.value == "Create New Label":
        result_label = result_label_input.value
    else:
        result_label = result_label_select.value

    if result_label:
        if result_label not in adata_g.obs.columns:
            adata_g.obs[result_label] = "NA"
    
        # ✅ ensure non-categorical type before assigning
        adata_g.obs[result_label] = adata_g.obs[result_label].astype(str)
        adata_g.obs.loc[adata_subset.obs_names, result_label] = adata_subset.obs["pred_cell_type"].astype(str)
        w_text_output(
            content=f"Results stored in metadata column '{result_label}'",
            appearance={"message_box": "success"}
        )
